...
path = r"D:\DataSet\0516_color" # 彩色圖片
path2 = r"D:\DataSet\0516_black" # 黑白圖片
path3 = r"D:\DataSet\0516_draft" # 線條圖片
### 資料預處理部分 ###
# 抓取資料路徑下的所有PNG圖片路徑
...
paths_subset = np.array(paths)[:10]
paths_subset2 = np.array(paths2)[:10]
paths_subset3 = np.array(paths3)[:10]
...
train_idxs = rand_idxs[:8]  # choosing the first 8000 as training set
val_idxs = rand_idxs[8:]  # choosing last 2000 as validation set
...
class ColorizationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        L = img_lab2[[0], ...] / 50. - 1.  # Between -1 and 1
...
def run():
    train_dl = make_dataloaders(paths=train_paths, paths2=train_paths3, split='train')
    val_dl = make_dataloaders(paths=val_paths, paths2=val_paths3, split='val')
    data = next(iter(train_dl))
    Ls, abs_ = data['L'], data['ab']
    print(Ls.shape, abs_.shape)
    print(len(train_dl), len(val_dl))
...
model = MainModel()
model = model.to(device)
model.load_state_dict(torch.load(r'D:\PythonProject\MangaUNet\OnePiece0516_3.pth'))
display_every=100
epochs=100
...
torch.save(model.state_dict(), 'OnePiece0516_4.pth')
