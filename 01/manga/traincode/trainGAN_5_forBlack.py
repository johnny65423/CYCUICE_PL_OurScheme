...
path3 = r"D:\DataSet\sketch_9000up\output0528" # 線條圖片
### 資料預處理部分 ###
# 抓取資料路徑下的所有PNG圖片路徑
...
paths_subset3 = np.array(paths3)[:100]
...
class MainModel(nn.Module):
    ...
    def backward_D(self):
        ...
        #print(999)
        #print(fake_image.detach().shape) # torch.Size([16, 2, 256, 256])
...
#model.load_state_dict(torch.load(r'D:\PythonProject\MangaUNet\OnePiece0520.pth')) # 如果要載入已經訓練好的模型使用這一行
display_every=100
epochs=100
...
for e in range(epochs):
    ...
    save_count += 1
    if save_num == 10:
        torch.save(model.state_dict(), 'OnePiece0712_temp.pth')
        f = open('save_count.txt', 'w')
        f.write(str(save_count))
        save_num = 0

torch.save(model.state_dict(), 'OnePiece0712.pth')
