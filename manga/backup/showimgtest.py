...
#    path = coco_path
# else:
#path = "D:/PythonProject/GANTestImg/train_manga"
path = r"E:\manga\black"
path2 = r"E:\manga\black"
...
np.random.seed(123)
paths_subset = np.array(paths)[:100]
paths_subset2 = np.array(paths2)[:100]
...
class ColorizationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ab = img_lab2[[1, 2], ...] / 110.  # Between -1 and 1
        if self.cuda:
            L = L.to(device)
            #RL = RL.to(device)
            ab = ab.to(device)
...
train_dl = make_dataloaders(paths=train_paths, paths2=train_paths2, split='train')
val_dl = make_dataloaders(paths=val_paths, paths2=val_paths2, split='val')
...
def visualize(model, data, save=True):
   ...
   matplotlib.image.imsave("output.jpg", fake_imgs[imgindex])
...
#model2 = torch.load(r'D:\PythonProject\MangaUNet\OnePiece0516.pth')
model2 = MainModel()
model2 = model2.to(device)
model2.load_state_dict(torch.load(r'E:\manga\OnePiece0530.pth'))
...
def change_img():
   visualize(model2, data, save=True)
   print(1)
...
label1= tk.Label(root, height=360, width=480, image=img1)
label1.grid(row=1,column=0,padx=20, pady=20, sticky="nw")
label2= tk.Label(root, height=360, width=480, image=img2)
label2.grid(row=1,column=0,padx=600, pady=20, sticky="nw")

#Create a Button to handle the update Image event
button= tk.Button(root, text= "Change", font= ('Helvetica 13 bold'),command= change_img)
button.grid(row=1, column=0, padx=500, pady=400, sticky="nw")
