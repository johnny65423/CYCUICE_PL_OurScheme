...
def mask_pic(img,mask):
    ...
    mask_mat = resize_img_1024(mask_mat)
    super_from = np.multiply(img, mask_mat)
    return super_from

def resize_img_1024(img):
    zeros = np.zeros((1024,1024,img.shape[2]), dtype=np.float)
    zeros[:img.shape[0], :img.shape[1]] = img
    return zeros

def resize_img_1024_3d(img):
...
def debug_pic_helper():
    for index in range(1130):
        ...
        mat_color=resize_img_1200(mat_color)
        ...
        mat_gray = resize_img_1200(mat_gray)
