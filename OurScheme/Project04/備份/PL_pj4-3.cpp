...
class Evaler {
  ...
  Token * Displaystr( Token * temp, map < string, TokenPtr > & localsymlist ) {
    ...
    if ( check->type != STRING )
      throw ArgNumError( "Display-String-type" ) ;
  ...
  Token * Symtostr( Token * temp, map < string, TokenPtr > & localsymlist ) {
    ...
    if ( check->type != SYMBOL )
      throw ArgNumError( "Symbol-String-type" ) ;
  ...
  Token * Numtostr( Token * temp, map < string, TokenPtr > & localsymlist ) {
    ...
    if ( check->type != INT && check->type != FLOAT )
      throw ArgNumError( "Number-String-type" ) ;
  ...
  float Add( Token * temp, bool & isfloat, map < string, TokenPtr > & localsymlist ) {
    if ( temp->left != NULL ) {
      Token * check ;
      try {
        check = Evalexp( temp->left, 1, localsymlist ) ;
  ...
  float Sub( Token * temp, bool & isfloat, map < string, TokenPtr > & localsymlist ) {
    if ( temp->left != NULL ) {
      Token * check ;
      try {
        check = Evalexp( temp->left, 1, localsymlist ) ;
  ...
  float Mul( Token * temp, bool & isfloat, map < string, TokenPtr > & localsymlist ) {
    if ( temp->left != NULL ) {
      Token * check ;
      try {
        check = Evalexp( temp->left, 1, localsymlist ) ;
  ...
  float Div( Token * temp, bool & isfloat, map < string, TokenPtr > & localsymlist ) {
    if ( temp->left != NULL ) {
      Token * check ;
      try {
        check = Evalexp( temp->left, 1, localsymlist ) ;
